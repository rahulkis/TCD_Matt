...
function UpdatePartnerAdmin() {
  const { id: partnerAdminId } = useParams();
  console.log({ partnerAdminId });
  ...
  useEffect(() => {
    editPartnerAdmin();
    setPageTitle('Edit Partner-Admin List By Id');
  }, []);

  const editPartnerAdmin = async () => {
    try {
      setLoading(true);
      await httpClient
        .get(`${PARTNER_ADMIN.EDIT_PARTNER_ADMIN.replace('{partnerAdminId}', partnerAdminId)}`)
        .then((res) => {
          console.log({ res });
          if (res.data.success) {
  ...
  const handleOnSubmit = async (e) => {
    ...
    try {
      if (partnerAdminId) {
        await httpClient
          .patch(`${PARTNER_ADMIN.UPDATE_PARTNER_ADMIN.replace('{partnerAdminId}', partnerAdminId)}`, formData)
          .then((res) => {
            if (res) {

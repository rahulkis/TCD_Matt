...
const getPartnerAdminList = async () => {
  try{
      setLoading(true);
      const response = await axios.get( API_BASE_URL + PARTNER.GET_PARTNER_ADMINS );
...
console.log({adminData});
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await httpClient
        .post(PARTNER.ADD_NEW_PARTNER, formData)
        .then((res) => {
          if (res.data.success) {
                        ...
                        >
                          ...
                          {adminData &&
                            adminData.map((data, i) => (
                              <option value={data.id} key={i}>{data.full_name}</option>
                          ))}

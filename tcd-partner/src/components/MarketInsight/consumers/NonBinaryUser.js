...
function NonBinaryUser({ state }) {
  ...
  const onChangeUpdate = async (type, changeToDate) => {
    ...
    if (changeToDate && changeToDate === "0") {
      ...
      onChangeGetNonBinaryConsumers(dateFrom, dateTo);
    }
  };

  const onChangeGetNonBinaryConsumers = async (dateFrom, dateTo) => {
    try {
      let age_entries = [];
      let purpose_entries = [];
      setLoading(true);
      const users = await httpClient.get(
        `${MARKETINSIGHT.GET_CONSUMERS}?from=${dateFrom}&to=${dateTo}&state=${state}&gender=Others`
      );
      setOtherUsers(users.data.consumers);
  ...
  const getNonBinaryConsumers = useCallback(async () => {
    onChangeGetNonBinaryConsumers(

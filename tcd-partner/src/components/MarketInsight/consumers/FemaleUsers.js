...
function FemaleUsers({ state }) {
  ...
  const onChangeUpdate = async (type, changeToDate) => {
    ...
    if (changeToDate && changeToDate === "0") {
      ...
      onChangeGetFemaleConsumers(dateFrom, dateTo);
    }
  };

  const onChangeGetFemaleConsumers = async (dateFrom, dateTo) => {
    let age_entries = [];
    let purpose_entries = [];
    try {
      setLoading(true);
      const users = await httpClient.get(
        `${MARKETINSIGHT.GET_CONSUMERS}?from=${dateFrom}&to=${dateTo}&state=${state}&gender=Female`
      );
      setFemaleUsers(users.data.consumers);
  ...
  const getFemaleConsumers = useCallback(async () => {
    onChangeGetFemaleConsumers(

...
function Symptoms() {
  ...
  const onChangeUpdate = async (type, changeToDate) => {
    ...
    if (changeToDate && changeToDate === "0") {
      ...
      onChangeGetTopSymptoms(dateFrom, dateTo);
    }
  };

  const onChangeGetTopSymptoms = async (dateFrom, dateTo) => {
    try {
      setLoading(true);
      const symptoms = await httpClient.get(
        `${MARKETINSIGHT.GET_TOP_SYMPTOMS}?from=${dateFrom}&to=${dateTo}`
      );
      setTopSymptoms(symptoms.data.symptoms_array);
    } catch (err) {
  ...
  const getTopSymptoms = useCallback(async () => {
    onChangeGetTopSymptoms(dateRangePickerDate.from, dateRangePickerDate.to);
  }, [dateRangePickerDate]);

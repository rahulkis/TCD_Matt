...
function TopActivities() {
  ...
  const onChangeUpdate = async (type, changeToDate) => {
    ...
    if (changeToDate && changeToDate === "0") {
      ...
      onChangeGetTopActivities(dateFrom, dateTo);
    }
  };
  const onChangeGetTopActivities = async (dateFrom, dateTo) => {
    try {
      setLoading(true);
      const activities = await httpClient.get(
        `${MARKETINSIGHT.GET_TOP_ACTIVITIES}?from=${dateFrom}&to=${dateTo}`
      );
      setTopActivities(activities.data.activities_array);
    } catch (err) {
  ...
  const getTopActivities = useCallback(async () => {
    onChangeGetTopActivities(dateRangePickerDate.from, dateRangePickerDate.to);
  }, [dateRangePickerDate]);

...
function HealthConditions() {
  ...
  const onChangeUpdate = async (type, changeToDate) => {
    ...
    if (changeToDate && changeToDate === "0") {
      ...
      onChangeGetTopHealthConditions(dateFrom, dateTo);
    }
  };

  const onChangeGetTopHealthConditions = async (dateFrom, dateTo) => {
    try {
      setLoading(true);
      const conditions = await httpClient.get(`${MARKETINSIGHT.GET_TOP_CONDITIONS}?from=${dateFrom}&to=${dateTo}`);
      setTopHealthConditions(conditions.data.conditions_array);
    } catch (err) {
  ...
  const getTopHealthConditions = useCallback(async () => {
    onChangeGetTopHealthConditions(dateRangePickerDate.from, dateRangePickerDate.to);
  }, [dateRangePickerDate]);


  const calculateGraphData = (categories) => {
    categories.sort((a, b) => {
      ...
      datasets: [
        {
          label: categories,
          data: data,
          backgroundColor: ['rgba(34, 64, 47, 1)', 'rgba(44, 99, 66, 1)', 'rgba(169, 201, 182, 0.57)', 'rgba(253, 204, 30, 1)', 'rgba(245, 158, 11, 1)'],
          borderColor: ['rgba(34, 64, 47, 1)', 'rgba(44, 99, 66, 1)', 'rgba(169, 201, 182, 0.57)', 'rgba(253, 204, 30, 1)', 'rgba(245, 158, 11, 1)'],
          borderWidth: 1,
        },
      ],
    };
        ...
        <div className="col-12">
          <div className="purpose_main">
            ...
            <div className="table-responsive mb-4 mb-0 text-left">
              <DataTable
                columns={columns}
                data={topHealthConditions}
                progressPending={isLoading}
                highlightOnHover
              />
            </div>

            {topHealthConditions.map((activity, index) => (
              activity.categories.length > 0 ? 
              <div className="purpose_list" key={index}>
                <h4> {activity.name} </h4>              
                <div className="progress_list">
                  <Bar options={options} data={calculateGraphData(activity.categories)} />
                </div>
